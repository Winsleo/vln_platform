...
projet_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
streamvln_path = os.path.join(os.path.dirname(projet_path), "StreamVLN")
sys.path.insert(0, streamvln_path)
from llava.model.language_model.llava_qwen import LlavaQwenModel
from llava.model.llava_arch import LlavaMetaForCausalLM
from .constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, MEMORY_TOKEN_INDEX
    ...
    def encode_rgbd(self, images, depths, poses, intrinsics, time_ids=None, task_ids=None):
        ...
        if num_view != 1:
            ...
            for b in range(batch_size):
                ...
                his_image_feature = self.get_model().mm_projector(his_image_feature)  # his_image_feature: torch.Size([8, 729, 3584])
                his_image_feature = self.get_2dPool(his_image_feature, 2) # his_image_feature: torch.Size([8, 196, 3584])
        ...
        for j, image_feature in enumerate(image_features):
            ...
            image_feature = self.get_model().mm_projector(image_feature) # image_feature: torch.Size([1, 729, 3584])
            image_feature = self.get_2dPool(image_feature, 2) # image_feature: torch.Size([1, 196, 3584])
