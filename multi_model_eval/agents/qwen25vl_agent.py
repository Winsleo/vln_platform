...
class Qwen25VLAgent(BaseAgent):
    ...
    def parse_actions(self, text: str) -> List[int]:
        ...
        events: List[Tuple[int, int]] = []  # (position, action_code) expanded later with counts
        ...
        for m in turn_pat.finditer(s):
            ...
            # store as negative code with multiply encoded in tuple list later
            events.append((pos, (code, steps)))
    ...
    def act(self, env_id: int, step_id: int, input_dict: Dict[str, Any]) -> int:
        ...
        if len(self.action_queue) > 0:
            ...
            print(f"Using queued action: {action}, queue length: {len(self.action_queue)}")
            return action
        ...
        with torch.inference_mode():
            ...
            outputs = self.processor.decode(output_ids, input_ids, skip_special_tokens=True)[0].strip()
        ...
        if len(actions) > 0:
            ...
            print(f"Queued {len(self.action_queue)} additional actions: {self.action_queue}")
